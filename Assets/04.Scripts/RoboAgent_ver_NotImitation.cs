...
public class RoboAgent_ver_NotImitation : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        for (int i = 0; i < 4; i++) action.Add(actions.ContinuousActions[index++]);
        //for (int i = 0; i < 4; i++) Debug.Log($"action{i} : {action[i]}");
        for (int i = 0; i < 4; i++)
            if (float.IsNaN(action[i]))
            {
                initEpisode = true;
                SetReward(-0.01f);
                EndEpisode();
        ...
        for (int i = 0;i < 55; i++){
            ...
            Quaternion signalQuat = new Quaternion(
                    Mathf.Clamp(action[0], -1, 1),
                    Mathf.Clamp(action[1], -1, 1),
                    Mathf.Clamp(action[2], -1, 1),
                    Mathf.Clamp(action[3], -1, 1)
                );
